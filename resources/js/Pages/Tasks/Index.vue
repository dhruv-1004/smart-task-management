...
console.log("tasksssss:",props.tasks);
...
console.log("User array:",props.user);
// console.log("Users ;",localUser.value)
console.log("Tasks ;",localTask.value)
...
console.log("user", props.tasks.user)
        ...
        border border-gray-200 dark:border-gray-700 rounded-lg p-6">
            ...
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
              ...
              text-gray-800 dark:text-gray-100
                ...
                <select v-model="statusFilters" class="sm:w-1/4 
              ...
              hover:bg-indigo-50 dark:hover:bg-gray-700">
                    <div class="flex flex-col sm:flex-row m:items-center sm:space-x-6 flex-1">
                        ...
                        <div class="mt-2 sm:mt-0 flex flex-wrap gap-4 text-sm text-gray-600">
                            <span class="px-3 py-1 rounded-full text-xs border font-semibold shadow-sm" 
                            :class="{
                            ...
                            :class="{
                              'bg-blue-200 text-blue-700 dark:bg-blue-700 dark:text-blue-300': task.status==='Pending'|| 'In-progress',

                              'bg-green-100 text-green-700 dark:bg-green-900 dark:text-green-300': task.status==='Completed'
                            }">{{ task.status }}</span>
                            
                            <span class="px-2 pt-1.5 pb-1 rounded-full text-xs font-medium border transition-all"
                            :class="{
...
class="fixed inset-0 bg-black/60 backdrop-blur-sm flex justify-center items-center z-50 p-4">
    ...
    scale-100">
        ...
        <form @submit.prevent="createTask" class="space-y-5">
            ...
            <!-- Assign task -->
             <div>
                <label class="block mb-1 text-sm font-medium text-gray-700 dark:text-gray-300">Assign Task</label>
                <select
                v-model="newTask.user_id" 
                class="px-3 py-2 bg-gray-100 dark:bg-gray-800 
                rounded-lg w-full border border-bg-gray-300
                 dark:border-gray-600 text-gray-800 dark:text-gray-100
                 focus:ring-2 focus:ring-indigo-500transition-all duration-200 " >
                    <option disabled value="" selected>--Assign task--</option>
                    <option v-for="user in props.user" :key="user.id" :value="user.id" >{{ user.name }}</option>
